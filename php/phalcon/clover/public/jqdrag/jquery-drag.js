    ...
    Constructor.prototype = {
        ...
        _move: function(eve) {
            if (this.is) {
                ...
                var width  = parseFloat($drag.css("width"));
                var height = parseFloat($drag.css("height"));
                ...
                // max
                if (max && max.left !== udf) {
                    if (to.left > (maxLeft = max.left + offset.left - width)) to.left = maxLeft;
                }
                if (max && max.top !== udf && to.top > max.top) {
                    if (to.top > (maxTop = max.top + offset.top - height)) to.top = maxTop;
                }
