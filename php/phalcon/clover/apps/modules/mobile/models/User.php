...
class User extends UserBase {
  ...
  //   	$follows = 'Psgod\Models\Follow';
		// return $builder->join($follows, "f.uid = u.uid and f.follow_who = {$uid} and f.status = ".self::STATUS_NORMAL, 'f')
		// 			   ->join($follows, "fr.uid = {$c_uid} and fr.follow_who = f.uid and fr.status = ".self::STATUS_NORMAL, 'fr', 'left')
		// 			   ->columns('u.uid, u.nickname, u.avatar, u.sex, fr.uid is_fans')
		// 			   ->orderBy('f.create_time desc')
		// 			   ->limit($limit, ($page-1)*$limit)
		// 			   ->getQuery()
		// 			   ->execute();
  ...
  //   	$builder = self::query_builder('u');
  //   	$follows = 'Psgod\Models\Follow';
		// return $builder->join($follows, "f.follow_who = u.uid and f.uid = {$uid} and f.status = ".self::STATUS_NORMAL, 'f')
		// 			   ->join($follows, "fr.uid = {$c_uid} and fr.follow_who = f.follow_who and fr.status = ".self::STATUS_NORMAL, 'fr', 'left')
		// 			   ->columns('u.uid, u.nickname, u.avatar, u.sex, fr.uid is_fans')
		// 			   ->orderBy('f.create_time desc')
		// 			   ->limit($limit, ($page-1)*$limit)
		// 			   ->getQuery()
    ...
    public static function getCollectionFocus($uid, $last_updated, $page, $limit=10) {
        ...
        $fields = "user.avatar, user.sex, user.uid, user.nickname ".
            ",item.id, item.create_time, item.update_time, item.desc ".
        ...
        $sql = "(SELECT $fields, item.reply_count, item.end_time, 1 type 
            FROM focuses f 
            LEFT JOIN asks item 
            ...
            UNION
                (SELECT $fields, 0 end_time, item.reply_count , 2 type 
            FROM collections c 
            LEFT JOIN asks item 
