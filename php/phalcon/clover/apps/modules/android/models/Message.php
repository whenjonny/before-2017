...
class Message extends MessageBase
{
	...
	public static function commentMsgList($uid, $last_updated, $page=1, $limit=10) {
        ...
        $where = array(
            'm.msg_type = '.self::TYPE_COMMENT,
            'm.status = '.self::STATUS_NORMAL,
            'm.receiver = '.$uid,
            'm.target_type='.self::TARGET_COMMENT,
        );
		return $builder->join($users, 'm.sender = u.uid', 'u', 'LEFT')
            ->join($comms, 'c.uid = m.sender AND c.id=m.target_id AND c.status = '.self::STATUS_NORMAL, 'c', 'LEFT')
